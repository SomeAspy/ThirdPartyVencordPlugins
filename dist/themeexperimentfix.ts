...
function getStaticFixes() {
    try {
        const searchFocused = findByProps('focused', 'queryText')
            .focused as string;
        const scrollerAuto = findByProps(
            'auto',
            'managedReactiveScroller',
        ).auto.split(' ')[0] as string;
        const { searchFilter, searchAnswer } = findByProps(
            'searchFilter',
            'searchAnswer',
        ...
        return {
            ...
            [`${scrollerAuto}::-webkit-scrollbar-track`]:
...
export default definePlugin({
    ...
    description:
        'Fixes parts of Discord that they forgot to make themeable via the "Client Themes" experiment',
    authors: [Devs.Vap],
    patches: [
        {
            find: 'clientThemeCSS',
    ...
    apply(rgbMap: Record<string, number[]>) {
        const cssBody = [...Object.entries(rgbMap)]
            .map(([k, v]) => `--${k}-rgb:${v.join(',')}`)
            .join(';');
        fixesStyle?.remove();
